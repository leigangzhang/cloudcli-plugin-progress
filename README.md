...
- **大会话轮询提取**：通过 `PROGRESS_USE_POLLING=true` 开启，把长会话按每 5 轮一组顺序交给 LLM 提取，避免单次 prompt 超出上下文窗口。
...
- **大会话提取失败**：
  ...
  - 轮询策略：在 `.env` 中设置 `PROGRESS_USE_POLLING=true`，插件会把会话切成每 5 轮一组顺序提取，降低单次 prompt 长度。
