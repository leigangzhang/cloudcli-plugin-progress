...
- **ProgressTree extraction** — Optional LLM mode uses a compact tree digest plus user text and a short assistant summary to update only affected nodes through a local patch merge. DeepSeek-compatible endpoints run with thinking disabled and JSON output constraints.
