...
function isProgressResponse(value) {
  const v2 = value;
  return typeof v2 === "object" && v2 !== null && typeof v2.tree === "object" && typeof v2.status === "string" && ["idle", "syncing", "error", "paused"].includes(v2.status);
}
...
function renderPlainDetails(label, text, colors) {
  ...
  return `
    <details style="margin:6px 0;color:${colors.text};">
      ...
      <pre style="margin:6px 0 0;padding:8px;max-height:300px;overflow:auto;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word;background:#000;color:#e5e6eb;border:1px solid ${colors.border};border-radius:6px;font-size:0.75rem;line-height:1.2em;">${escapeHtml2(text)}</pre>
    </details>
  `;
}
