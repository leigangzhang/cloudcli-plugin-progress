  ...
  return `
    <div class="pp-goal" data-goal-id="${escapeHtml(goal.id)}">
      <div class="pp-goal-header pp-tooltip" data-tooltip="${description}" style="position:relative;display:flex;align-items:center;gap:9px;padding:11px 15% 11px 12px;border-bottom:1px solid ${colors.divider};background:${colors.surface};cursor:pointer;">
        ...
        ${steps.length > 0
          ? `<button type="button" class="pp-goal-toggle" data-goal-toggle-id="${escapeHtml(goal.id)}" title="${goalToggleTitle}" style="position:absolute;right:6%;top:50%;transform:translateY(-50%);padding:2px 9px;background:${colors.accent};border:1px solid ${colors.accent};border-radius:5px;color:#fff;font-size:0.8rem;font-weight:700;line-height:1.2;">${goalFullyExpanded ? '-' : '+'}</button>`
          : ''}
      </div>
...
function renderStep(
  ...
  const rowStyle = embedded
    ...
    : `padding:10px 7% 10px 12px;border-bottom:1px solid ${colors.divider};`;
