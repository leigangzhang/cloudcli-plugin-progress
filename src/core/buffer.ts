...
import { buildTurns, type LogEntryWithLine } from './turns.js';
...
export class ConversationBuffer {
  ...
  private lineNumbers = new WeakMap<LogEntry, number>();
  ...
  push(entry: LogEntry): void {
    ...
    this.lineCounter++;
    this.lineNumbers.set(entry, this.lineCounter);
  ...
  getTurns(): import('./types.js').ConversationTurn[] {
    const withLines: LogEntryWithLine[] = [];
    for (const entry of this.entries) {
      const line = this.lineNumbers.get(entry);
      if (line === undefined) continue;
      withLines.push({ entry, lineNumber: line });
    }
    return buildTurns(withLines);
  }
