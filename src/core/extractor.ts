...
import type { ConversationTurn, LLMConfig, LLMExtractionEngine, ProgressGoal, ProgressStatus, ProgressTree } from './types.js';
...
function buildMergePrompt(trees: ProgressTree[]): string {
...
function concatenateTrees(trees: ProgressTree[]): ProgressTree {
  const goalMap = new Map<string, ProgressGoal>();
  let version = 0;
  for (const tree of trees) {
    ...
    for (const goal of tree.goals) {
      ...
      if (existing) {
        const stepMap = new Map<string, import('./types.js').ProgressStep>();
        for (const step of existing.steps ?? []) stepMap.set(step.id, step);
        for (const step of goal.steps ?? []) stepMap.set(step.id, step);
...
export class LLMExtractionEngineImpl implements LLMExtractionEngine {
  ...
  private async extractByPolling(tree: ProgressTree, turns: ConversationTurn[]): Promise<ProgressTree> {
    ...
    const partialTrees: ProgressTree[] = [];
    for (const chunk of chunks) {
      const partial = await this.doExtract(
        { version: partialTrees.length, goals: [] },
        summarizeTurns(chunk, chunkSize),
        false,
      );
  ...
  private async doMerge(trees: ProgressTree[]): Promise<ProgressTree> {
    const response = await this.client.messages.create({
      model: this.config.model,
      max_tokens: this.config.maxTokens ?? 4096,
